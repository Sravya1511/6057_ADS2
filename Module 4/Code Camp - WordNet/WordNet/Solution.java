...
final class Solution {
     ...
     */
		public static void main(final String[] args) {
			...
			HashMap<String, ArrayList<Integer>> synsetQueries =
            new HashMap<String, ArrayList<Integer>>();

			try {
			    Scanner scanner = new
                Scanner(new File("Files/" + x));
                ...
                for (int i: synset.keySet()) {
                	...
                	for (int j = 0; j < keys.length; j++) {
                		if (synsetQueries.containsKey(keys[j])) {
                            ...
                            synsetQueries.get(keys[j]);
                			intarr.add(i);
                			synsetQueries.put(keys[j], intarr);
                		} else {
                			ArrayList<Integer> indx =
            ...
            DirectedCycle directedCycle = new DirectedCycle(digraph);
                switch (s) {
                	...
                	while (input.hasNext()) {
                		...
                		sap.length(synsetQueries.get(tokens[0]),
                            synsetQueries.get(tokens[1]), synset);
